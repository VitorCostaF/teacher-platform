...
import type { FonteConteudo, NivelDificuldade, ProvaConfig, TipoQuestao } from '../types'
...
const TIPOS_QUESTAO: { tipo: TipoQuestao; label: string }[] = [
...
export function ProvaConfigPanel({
  config, isGenerating, isUploading, avisoConteudo, podeGerar,
  onChange, onGerar, onUpload,
}: Props) {
  ...
  return (
    <div className="flex flex-col gap-4 overflow-y-auto">
      <div>
        <h2 className="text-base font-semibold text-gray-900">Configurar prova</h2>
        <p className="mt-0.5 text-xs text-gray-500">Defina os parâmetros e gere as questões com IA</p>
      </div>
      ...
      {/* Título */}
      <div className="flex flex-col gap-1.5">
        <label className="text-xs font-medium text-gray-700">Título da prova</label>
        <input
          type="text"
          value={config.titulo}
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="flex flex-col gap-1.5">
          <label className="text-xs font-medium text-gray-700">Duração (min)</label>
          <input
            type="number"
            min={10}
            max={300}
      ...
      <div className="flex flex-col gap-2">
        ...
        <div className="flex flex-col gap-2">
          {TIPOS_QUESTAO.map(({ tipo, label }) => {
            const qtd = config.quantidades[tipo] ?? 0
            const ativo = qtd > 0
            return (
              <div key={tipo} className="flex items-center gap-3">
                <input
                  ...
                  onChange={e => {
                    ...
                    if (e.target.checked) novas[tipo] = 5
                    else delete novas[tipo]
                ...
                {ativo && (
                  <input
                    ...
                    onChange={e => {
                      ...
                      onChange('quantidades', { ...config.quantidades, [tipo]: n })
                    }}
